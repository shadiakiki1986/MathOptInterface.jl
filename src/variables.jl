...
By default, this function falls back to created a free variable with
[`add_variable`](@ref) and then constrain it to belong to `set` with
...
* if `model` does not support created free variables, it should only implement
...
By default, this function falls back to created free variables with
[`add_variables`](@ref) and then constrain it to belong to `set` with
[`add_constraint`](@ref). Therefore, there is no need to implement this function
if `model` supports creating free variables and supports transforming free
variables into variables in `set`. However,

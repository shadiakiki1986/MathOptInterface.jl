...
"""
    AbstractToIntervalBridge{T, S1, F, G}
...
"""
    GreaterToIntervalBridge{T, F<:MOI.AbstractScalarFunction, G<:MOI.AbstractScalarFunction} <:
        AbstractToIntervalBridge{T, MOI.GreaterThan{T}}

Transforms a `G`-in-`GreaterThan{T}` constraint into an `F`-in-`Interval{T}`
constraint.
"""
struct GreaterToIntervalBridge{T, F<:MOI.AbstractScalarFunction, G<:MOI.AbstractScalarFunction} <:
    AbstractToIntervalBridge{T, MOI.GreaterThan{T}, MOI.Interval{T}, F, G}
...
function concrete_bridge_type(::Type{<:GreaterToIntervalBridge{T}},
    ...
    return GreaterToIntervalBridge{T, F, G}
end

"""
    LessToIntervalBridge{T, F<:MOI.AbstractScalarFunction, G<:MOI.AbstractScalarFunction} <:
        AbstractToIntervalBridge{T, MOI.LessThan{T}}

Transforms a `G`-in-`LessThan{T}` constraint into an `F`-in-`Interval{T}`
constraint.
"""
struct LessToIntervalBridge{T, F<:MOI.AbstractScalarFunction, G<:MOI.AbstractScalarFunction} <:
    AbstractToIntervalBridge{T, MOI.LessThan{T}, MOI.Interval{T}, F, G}
    constraint::CI{F, MOI.Interval{T}}
end
...
function concrete_bridge_type(::Type{<:LessToIntervalBridge{T}},
    ...
    F = MOIU.promote_operation(+, T, G)
    return LessToIntervalBridge{T, F, G}
end
